...
// const web_link = 'https://127.0.0.1:8000/auth/'
  ...
  if (startPayload && startPayload != user_id) {
    ...
    connection.query(
      ...
      (error, results, fields) => {
        if (results.length === 0) {
          connection.query(
            'SELECT * FROM reffers WHERE `user_id` = ? AND `reffer_id`',
            [user_id, reffer_id],
            (error, results, fields) => {
              // if (error) {
              ...
              if (results.length === 0) {
                connection.query(
                  'INSERT INTO reffers (`user_id`, `reffer_id`) VALUES (?, ?)',
                  [user_id, reffer_id],
                  (insertError, insertResults, insertFields) => {
                    ...
                    connection.query(
                      ...
                      [200, reffer_id],
                      (coinUpdateError, coinUpdateResults) => {
                        // if (coinUpdateError) {
                    ...
                    // update wallet
                    connection.query(
                      'INSERT INTO `wallet`(`user_id`, `coin`, `type`, `title`) VALUES (? , ? , ? , ?)',
                      [reffer_id, 200, 'Referral', user_id],
                      (coinUpdateError, coinUpdateResults) => {
                        // if (coinUpdateError) {
                        //     console.error('Error updating user coins:', coinUpdateError);
                        //     return res.status(500).send('Error updating user coins');
...
io.on('connection', (socket) => {
  ...
  socket.on('sendMessage', ({ room, message }) => {
    io.to(room).emit('receiveMessage', {
      message,
      sender: users[socket.id] || socket.id,
    })
    console.log(
      `Message sent to room ${room} from ${users[socket.id]}: ${message}`
    )
  })
